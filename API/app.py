...
class FetchAllLeaveResource(Resource):
    def get(self):
        try:
            # Fetch all leave applications from MongoDB
            leave_applications = list(collection.find())

            return jsonify(leave_applications), 200

        except Exception as e:
            print(f"Error: {e}")
            return {'message': 'Failed to fetch leave applications'}, 500
        

api.add_resource(RegisterResource, '/api/register')
api.add_resource(LoginResource, '/api/login')
api.add_resource(ApplyLeaveResource, '/api/apply-leave')
api.add_resource(FetchAllLeaveResource, '/api/fetch_all_leaves')
